	...
	context 'changes' do
		...
		before(:all) do
			...
			it 'finds audit archives for options: {audit_archives: true, layout: "long"}' do
				full_change_keys = @changes_with_archives.sample.keys
				basic_change_keys = @changes.sample.keys
				expect(basic_change_keys.size).to be < full_change_keys.size
				expect(full_change_keys).to include('audits')
			end
